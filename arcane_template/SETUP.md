# Arcane Template Setup Guide

This guide will walk you through setting up a complete Flutter application with the Arcane UI framework, following a 3-project architecture (client app, shared models, and server).
...
This template uses a 3-project architecture:
...
- **Code sharing**: Models package is used by both client and server
- **Type safety**: Shared models ensure consistency across client and server
- **Scalability**: Server can be deployed independently to Google Cloud Run
...
# 2. Add Arcane dependencies
cd my_app
flutter pub add arcane toxic toxic_flutter pylon rxdart hive hive_flutter

# 3. Generate icons and splash
dart run flutter_launcher_icons
dart run flutter_native_splash:create
...
1. Copy template code from `arcane_template/lib/` to your app
2. Customize screens, routes, and services for your needs
3. Add your custom models to `YOUR_APP_NAME_models/lib/models/`
4. Implement server API endpoints in `YOUR_APP_NAME_server/lib/server/api/`
5. Configure authentication providers in Firebase (if using Firebase)
6. Test on multiple platforms
7. Deploy to app stores and hosting
